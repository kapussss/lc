...
const MARKOV_FILE = 'markov_chain.json';
const ONLINE_WEIGHTS_FILE = 'online_weights.json';
const PERCEPTRON_FILE = 'perceptron_model.json';
...
class MarkovChain {
  ...
  save(filePath) {
    try {
      fs.writeFileSync(filePath, JSON.stringify({ order: this.order, transitions: this.transitions }));
    } catch (e) { console.error('[Markov] Save error:', e.message); }
  }
  ...
  load(filePath) {
    ...
    } catch (e) { console.error('[Markov] Load error:', e.message); }
  }
}

// ==================== CUSUM DETECTOR ====================
class CUSUMDetector {
  constructor(threshold = 1.2) {
  ...
  update(actualTai, expectedProb = 0.5) {
    const z = (actualTai ? 1 : 0) - expectedProb;
    this.posSum = Math.max(0, this.posSum + z - 0.05);
    this.negSum = Math.max(0, this.negSum - z - 0.05);
    
    if (this.posSum > this.threshold) {
      this.posSum = 0;
      this.lastSignal = 'Tài';
      return 'Tài';
    }
    if (this.negSum > this.threshold) {
      this.negSum = 0;
      this.lastSignal = 'Xỉu';
      return 'Xỉu';
    }
  ...
  save() {
    return { posSum: this.posSum, negSum: this.negSum, lastSignal: this.lastSignal };
  }

  load(data) {
    if (data) {
      this.posSum = data.posSum || 0;
      this.negSum = data.negSum || 0;
...
class OnlineEnsemble {
  constructor(numModels) {
    this.weights = new Array(numModels).fill(1 / numModels);
    this.lr = CONFIG.ONLINE_LR;
  }

  predict(modelOutputs) {
    if (!modelOutputs || modelOutputs.length === 0) return 0;
    
    let score = 0;
    let totalWeight = 0;
    modelOutputs.forEach((out, i) => {
      const w = this.weights[i] || 0;
      if (out.prediction === 'Tài') score += w;
      else if (out.prediction === 'Xỉu') score -= w;
      totalWeight += w;
    });
    return totalWeight > 0 ? score / totalWeight : 0;
  }

  update(modelOutputs, actualTai) {
    if (!modelOutputs || modelOutputs.length === 0) return;
    
    // Ensure weights array matches model count
    while (this.weights.length < modelOutputs.length) {
      this.weights.push(1 / modelOutputs.length);
    }
    
    const predScore = this.predict(modelOutputs);
    const target = actualTai ? 1 : -1;
    const error = target - predScore;
    
    modelOutputs.forEach((out, i) => {
      const signal = out.prediction === 'Tài' ? 1 : -1;
      const grad = -error * signal;
      this.weights[i] = Math.max(0.01, this.weights[i] - this.lr * grad);
    });
    ...
    if (sum > 0) {
      ...
      this.weights = new Array(modelOutputs.length).fill(1 / modelOutputs.length);
    }
  }

  save() {
    return { weights: this.weights, lr: this.lr };
  }

  load(data) {
    if (data) {
      this.weights = data.weights || this.weights;
      this.lr = data.lr || CONFIG.ONLINE_LR;
    }
  }
}
...
class SimplePerceptron {
  ...
  predict(features) {
    if (!features || features.length !== this.inputSize) return 0.5;
  ...
  train(features, target) {
    ...
    for (let i = 0; i < this.inputSize; i++) {
      this.weights[i] += this.lr * grad * (features[i] || 0);
    }
  ...
  load(data) {
    if (data) {
      ...
      this.lr = data.lr || CONFIG.PERCEPTRON_LR;
      // Resize weights if needed
      while (this.weights.length < this.inputSize) {
        this.weights.push(Math.random() * 0.2 - 0.1);
      }
      this.weights = this.weights.slice(0, this.inputSize);
    }
  }
}
...
class AnomalyDetector {
  constructor() {
    ...
    this.cusumState = { posSum: 0, negSum: 0 };
  }
  ...
  loadAnomalyData() {
    try {
      if (fs.existsSync(ANOMALY_FILE)) {
        ...
        this.cusumState = parsed.cusumState || this.cusumState;
...
let cusum = { hu: new CUSUMDetector(CONFIG.CUSUM_THRESHOLD), md5: new CUSUMDetector(CONFIG.CUSUM_THRESHOLD) };
let onlineEnsemble = { hu: new OnlineEnsemble(8), md5: new OnlineEnsemble(8) };
let perceptronFallback = { hu: new SimplePerceptron(12), md5: new SimplePerceptron(12) };
...
function extractFeaturesForPerceptron(results, sums, hour, biasCorrection) {
  const last5 = results.slice(0, 5);
  ...
  const altLen = anomalyDetector.getAlternatingLength(results);
  
  return [
    taiCount5 / 5,
    streak / 10,
    Math.abs(sumMean - 10.5) / 4,
    sumStd / 3,
    hour / 24,
    (results[0] === 'Tài' ? 1 : 0),
    (results.length > 1 && results[1] === 'Tài' ? 1 : 0),
    (results.length > 2 && results[2] === 'Tài' ? 1 : 0),
    (taiCount5 >= 3 ? 1 : 0),
    (sumMean > 10.5 ? 1 : 0),
    (biasCorrection + 0.5),
    altLen / 10
...
function saveModelStates() {
  try {
    // Save Markov chains
    markovChain.hu.save('markov_hu.json');
    markovChain.md5.save('markov_md5.json');
    
    // Save Online Ensemble weights
    fs.writeFileSync(ONLINE_WEIGHTS_FILE, JSON.stringify({
      hu: onlineEnsemble.hu.save(),
      md5: onlineEnsemble.md5.save(),
      timestamp: new Date().toISOString()
    }));
    
    // Save Perceptron models
    fs.writeFileSync(PERCEPTRON_FILE, JSON.stringify({
      hu: perceptronFallback.hu.save(),
      md5: perceptronFallback.md5.save(),
      timestamp: new Date().toISOString()
    }));
    
    // Save CUSUM states
    fs.writeFileSync('cusum_state.json', JSON.stringify({
      hu: cusum.hu.save(),
      md5: cusum.md5.save(),
      timestamp: new Date().toISOString()
    }));
  } catch (error) {
    console.error('Error saving model states:', error.message);
  }
}
...
function loadModelStates() {
  try {
    // Load Markov chains
    markovChain.hu.load('markov_hu.json');
    markovChain.md5.load('markov_md5.json');
    
    // Load Online Ensemble weights
    if (fs.existsSync(ONLINE_WEIGHTS_FILE)) {
      const data = JSON.parse(fs.readFileSync(ONLINE_WEIGHTS_FILE, 'utf8'));
      if (data.hu) onlineEnsemble.hu.load(data.hu);
      if (data.md5) onlineEnsemble.md5.load(data.md5);
      console.log('[OnlineEnsemble] Loaded');
    }
    
    // Load Perceptron models
    if (fs.existsSync(PERCEPTRON_FILE)) {
      const data = JSON.parse(fs.readFileSync(PERCEPTRON_FILE, 'utf8'));
      if (data.hu) perceptronFallback.hu.load(data.hu);
      if (data.md5) perceptronFallback.md5.load(data.md5);
      console.log('[Perceptron] Loaded');
    }
    
    // Load CUSUM states
    if (fs.existsSync('cusum_state.json')) {
      const data = JSON.parse(fs.readFileSync('cusum_state.json', 'utf8'));
      if (data.hu) cusum.hu.load(data.hu);
      if (data.md5) cusum.md5.load(data.md5);
      console.log('[CUSUM] Loaded');
    }
  } catch (error) {
    console.error('Error loading model states:', error.message);
  }
}
...
async function verifyPredictions(type, currentData) {
  ...
  for (const pred of learningData[type].predictions) {
    ...
    if (actualResult) {
      ...
      // Update CUSUM
      cusum[type].update(actualResult.Ket_qua === 'Tài');

      // Update Online Ensemble
      if (pred.subModelOutputs && pred.subModelOutputs.length > 0) {
        onlineEnsemble[type].update(pred.subModelOutputs, pred.actual === 'Tài');
      }

      // Update Perceptron
      if (pred.features) {
        perceptronFallback[type].train(pred.features, pred.actual === 'Tài' ? 1 : 0);
      }
...
function checkAndResetIfNeeded(type) {
  const recent = learningData[type].recentAccuracy.slice(-50);
  if (recent.length >= 50) {
    const acc = recent.reduce((a, b) => a + b, 0) / recent.length;
    if (acc < CONFIG.RESET_THRESHOLD_ACC) {
      console.log(`⚠️ Accuracy ${type} below ${CONFIG.RESET_THRESHOLD_ACC * 100}%, resetting...`);
      resetType(type);
    }
  }
}
...
function resetType(type) {
  ...
  markovChain[type] = new MarkovChain(CONFIG.MARKOV_ORDER);
  cusum[type] = new CUSUMDetector(CONFIG.CUSUM_THRESHOLD);
  onlineEnsemble[type] = new OnlineEnsemble(8);
  perceptronFallback[type] = new SimplePerceptron(12);
  predictionProbBuffer[type] = [0.5, 0.5, 0.5, 0.5, 0.5];

  saveLearningData();
  saveModelStates();
  console.log(`[Reset] ${type} data reset complete`);
}
...
function calculateSuperPrediction(data, type) {
  ...
  // 4. CUSUM Signal
  const cusumSignal = cusum[type].update(results[0] === 'Tài');
  if (cusumSignal) {
    subModels.push({
      prediction: cusumSignal,
      confidence: 62,
      name: 'cusum'
    });
    factors.push(`CUSUM: ${cusumSignal}`);
  }
  ...
  if (monteCarloSimulators[type]) {
    ...
    if (mc && mc.prediction) {
      subModels.push({ prediction: mc.prediction, confidence: mc.confidence, name: 'mc' });
  ...
  // Prepare outputs for online ensemble
  const outputs = subModels.map(m => ({
    prediction: m.prediction,
    confidence: m.confidence
  }));

  // If we have sub models, use online ensemble
  let finalPrediction, rawConf;
  
  if (subModels.length >= 2) {
    // Use online ensemble
    const ensembleScore = onlineEnsemble[type].predict(outputs);
    finalPrediction = ensembleScore >= 0 ? 'Tài' : 'Xỉu';
    ...
    rawConf = Math.round(50 + Math.abs(ensembleScore) * 60) + agreementBonus;
  } else {
    // Use perceptron fallback
    const features = extractFeaturesForPerceptron(results, sums, currentHour, biasCorrection);
    const probTai = perceptronFallback[type].predict(features);
    finalPrediction = probTai > 0.5 ? 'Tài' : 'Xỉu';
    rawConf = Math.round(50 + Math.abs(probTai - 0.5) * 80);
  }

  // Clamp confidence
  rawConf = Math.min(CONFIG.MAX_CONFIDENCE, Math.max(CONFIG.FALLBACK_CONFIDENCE, rawConf));
  ...
  if (Math.abs(smoothedProb - 0.5) > 0.02) {
    finalPrediction = smoothedProb > 0.5 ? 'Tài' : 'Xỉu';
  }

  // Calibrate final confidence
  const finalConfidence = Math.round(anomalyDetector.calibrateConfidence(rawConf, 'ensemble'));
  
  // Extract features for later training
  const features = extractFeaturesForPerceptron(results, sums, currentHour, biasCorrection);

  return {
    prediction: finalPrediction,
    confidence: Math.max(CONFIG.FALLBACK_CONFIDENCE, finalConfidence),
    factors,
    allPredictions: subModels,
    methodsUsed: Object.fromEntries(subModels.map(m => [m.name, true])),
    subModelOutputs: outputs,
    features
  };
}
    ...
    if (dataHu && dataHu.length > 0) {
      ...
      if (lastProcessedPhien.hu !== nextHuPhien) {
        ...
        recordPrediction('hu', nextHuPhien, result.prediction, result.confidence,
          result.factors, result.methodsUsed, result.subModelOutputs, result.features);
...
app.get('/lc79-hu/analysis', async (req, res) => {
  try {
    ...
    res.json({
      ...
      ensembleWeights: onlineEnsemble.hu.weights,
...
app.get('/lc79-md5/analysis', async (req, res) => {
  try {
    ...
    res.json({
      ...
      factors: result.factors,
      methods: result.methodsUsed,
      ensembleWeights: onlineEnsemble.md5.weights,
      details: result.allPredictions?.map(p => ({
        method: p.name,
        prediction: p.prediction,
        confidence: p.confidence
...
app.get('/reset', (req, res) => {
  ...
  cusum = { hu: new CUSUMDetector(CONFIG.CUSUM_THRESHOLD), md5: new CUSUMDetector(CONFIG.CUSUM_THRESHOLD) };
  onlineEnsemble = { hu: new OnlineEnsemble(8), md5: new OnlineEnsemble(8) };
  perceptronFallback = { hu: new SimplePerceptron(12), md5: new SimplePerceptron(12) };
  metaModelUpdated = 0;
...
setInterval(() => saveModelStates(), 300000);

app.listen(PORT, '0.0.0.0', () => {
  console.log(`\n╔══════════════════════════════════════════════════════════════════╗`);
  console.log(`║     LẨU CUA 79 - AI PREDICTION v10.0 - ALWAYS-ON SYSTEM       ║`);
  console.log(`║  MC | Anomaly | QL | Dice | Markov | CUSUM | Perceptron | OL   ║`);
  console.log(`╚══════════════════════════════════════════════════════════════════╝\n`);
  console.log(`Server running: http://0.0.0.0:${PORT}`);
  console.log(`\n🚀 KEY FEATURES (v10.0):`);
  console.log(`  ✅ Always delivers prediction (never skips)`);
  console.log(`  ✅ Minimum confidence: ${CONFIG.FALLBACK_CONFIDENCE}%`);
  console.log(`  ✅ Ensemble of up to 9 methods with online learning`);
  console.log(`  ✅ Markov Chain (order ${CONFIG.MARKOV_ORDER}) for pattern detection`);
  console.log(`  ✅ CUSUM for early trend change detection`);
  console.log(`  ✅ Perceptron fallback when few models active`);
  console.log(`  ✅ Online Gradient Descent for weight optimization`);
  ...
  console.log(`  ✅ Auto-reset when accuracy drops below ${CONFIG.RESET_THRESHOLD_ACC * 100}%`);
  console.log(`\n📋 ENDPOINTS:`);
